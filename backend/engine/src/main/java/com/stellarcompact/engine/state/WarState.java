...
/**
 * A first-class state of open war between two factions (game-design 04/05). War is
 * the positive precondition that legalises a kinetic action against a
 * <em>non-neutral</em> target: absence of a forbidding treaty is necessary but
 * <em>not sufficient</em> - a faction must not strike one it is at peace-but-not-treaty
 * with. {@link com.stellarcompact.engine.validation.ActionValidator} consults the
 * active wars to gate {@code Attack}/{@code Blockade}/{@code Raid} (the gate is the
 * job of the kinetic cards; this card E1-12 introduces the war-state record and the
 * {@code DeclareWar} transition that sets it).
 *
