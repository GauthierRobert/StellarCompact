...
- **Floating origin:** at extreme zoom, world coordinates are re-centred on the camera periodically to avoid float32 precision breakdown (essential for "infinite" descent; deliberately omitted from the PoC for readability). **Implemented (E8-07).** The `CameraStore` holds a float64 `origin` signal and re-bases it onto the rendered camera whenever the camera drifts more than `ORIGIN_REBASE_PX` (16384) *pixels* from it (`maybeRebaseOrigin`, run each `stepFrame`). Because the threshold is in pixels it self-adapts to zoom — it essentially never fires zoomed out (origin stays `(0,0)`, identical to pre-E8-07) and re-bases readily at deep zoom where float32 would otherwise quantise. The `WebglDrawLayer` subtracts the origin (carried on `ViewTransform.originX/Y`) from **both** the per-instance star positions and the recovered camera world *in float64 on the CPU* before they are narrowed into the float32 instance buffer / `uCamWorld` uniform, so the shader's `(aWorld - uCamWorld) * scalePx` runs on small magnitudes. Subtracting the same origin from both operands leaves the on-screen result unchanged, so a re-base is **visually transparent** (the CPU `w2s`/`s2w` transforms work in float64 and ignore the origin — they are byte-identical across a re-base). The nebula pass keeps the absolute camera world: it needs galaxy-centre distance and only renders zoomed-out, where the origin is `(0,0)`.
...
- **Pre-generation / warming (E8-07):** for a launched galaxy, coarse tiles and the active region's fine tiles are pre-baked so the common views are warm. The api-layer `TilePrebakeService` warms (a) every coarse aggregate level `0..STAR_LIST_MIN_LEVEL-1` (a fixed, tiny set — 21 tiles for `STAR_LIST_MIN_LEVEL = 3`) and (b) the fine tiles overlapping an active-region world bbox at a chosen star-list level, hard-capped by `MAX_ACTIVE_TILES` (1024) so it never iterates the catalog. Warming is just `TileCache.get`, so it populates the same generate-on-miss cache the `TileController` reads — subsequent live `GET .../tile/...` (and the CDN in front) are cache hits, and the pre-bake adds **nothing** to the served payload or its `Cache-Control: public, max-age=31536000, immutable` + ETag headers. Trigger: `POST /api/galaxy/{gameSeed}/prebake?level&minX&minY&maxX&maxY` (admin/launch-time, `Cache-Control: no-store`).
