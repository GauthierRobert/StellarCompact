...
- `INSUFFICIENT_RESOURCES`, `NOT_OWNED`, `NOT_ADJACENT`, `ALREADY_REVEALED`, `NO_PATH`, `TREATY_FORBIDS` (with treaty id), `TECH_PREREQ_MISSING`, `NO_FREE_SLOT`, `NOT_AT_WAR`, `TARGET_UNKNOWN`, `OFFER_EXPIRED`.

**Re-prompt reflection is sanitized (X-01 finding X01-1).** A rejection message is built from typed ids/enums, but a few validator messages interpolate an agent-supplied free-string verbatim (e.g. `BuildFleet.shipSpec`, a self-supplied `SystemId`), bounded only by the parser's 32 KiB string cap. Before any engine message enters the single re-prompt, the agent-runtime re-prompt builder (`RejectionFeedback`) **strips CR/LF and other control characters and caps the message length (≤200 chars)**, so reflected agent text can never restructure the prompt (newline-led "instruction" injection). The blast radius was always same-agent-only (never another faction, never hidden state); this removes it. The stable `[code]` prefix is always machine-controlled.
...
- **Redundant Explore hygiene (E10-06 — IMPLEMENTED).** `Explore` of a system the actor has **already revealed/explored** is rejected `ALREADY_REVEALED`; a first-time `Explore` still passes. This closes the "valid no-op" the 3-agent sim flagged (finding F1: ~2000 wasted resolver slots/hour from bots re-exploring the same revealed neutral every tick), and keeps the action log meaningful. To make "already revealed" checkable in the **pure** engine (independent of the orchestrator's fog `WorldView`, which is a projection the engine never sees), the per-faction known map is now first-class engine state: `Faction.exploredSystems` is a `Set<SystemId>` of the systems that faction has revealed (game-design 03 Explore: "Adds it to the faction's known map"). The `Explore` resolution step records the target into the acting faction's set via `Faction.withExplored(SystemId)` — deterministically and **seed-independently** (a pure function of the validated action log; idempotent on re-reveal, monotone within a match), so the replay contract is preserved (the only state change is the intended reveal). The set is held **on the exploring faction** (the same pattern as E1-13 `revealedIntel`) rather than as a new `GameState` component, keeping the snapshot shape stable; the canonical hash sorts it so set order never leaks. **Security (no cross-faction leak).** The validator reads **only the actor's own `exploredSystems`** to decide `ALREADY_REVEALED`. The message names only the system id the actor itself supplied and the fact that it is on the actor's own known map — never another faction's state, never ownership, never anything hidden. The check therefore cannot be used as a fog oracle; the existence check still reports `TARGET_UNKNOWN` first for a system not in the snapshot. The closed-I/O contract is unchanged: a rejection still feeds exactly one re-prompt, then Hold.
...
**Four-phase tick orchestration (E4-05).** `orchestrator`, package `com.stellarcompact.orchestrator.tick`: `TickOrchestrator.runTick(List<Seat>, TickContext) → TickResult` drives one real-time tick through **Perception → Negotiation → Action → Resolution**. The agent-calling phases (Negotiation, then Action) fan out with **one virtual thread per `Seat`** under a **shared per-phase deadline** using a Java 25 `StructuredTaskScope` opened with `Joiner.awaitAll()` and `withTimeout(perPhaseDeadline)`. When the deadline elapses the scope cancels (interrupts) every still-running subtask; **a straggler that misses the deadline contributes `Hold`** (an empty `SeatDecision`) and the tick proceeds with the survivors — a slow agent never stalls the tick. (Each subtask records its `SeatDecision` into a side-channel results map before returning, because after a `TimeoutException` the scope is not in the joined state and `Subtask.get()` is unavailable; a seat absent from the map is a cancelled straggler → `Hold`.) A `Seat` is either a `ScriptedSeat` (an E3-01 `Sovereign` bot, validated by the orchestrator) or an `LlmSeat` (the production path: build the fog-filtered `WorldView` via `WorldViewBuilder` (E3-02) → assemble the prompt via `PromptAssembler` (E4-02) → run `ActionValidationLoop.decide(ChatClient, AssembledPrompt, ValidationContext)` (E4-04), the `ChatClient` obtained only via `SovereignChatClientFactory` (E4-01) — no vendor type in the orchestrator). **Resolution is single-threaded and pure:** the gathered decisions are reassembled into a *deterministic* `SubmittedAction` batch (seats sorted by faction id, each seat's surviving actions in submission order, so the batch never depends on which virtual thread finished first) and folded through `Resolver.resolveResult(...)` exactly once with the constant `gameSeed` (the resolver folds the tick in itself); concurrency lives only in the gathering. The tick clock is advanced by the caller (the source of per-tick RNG variation), mirroring the headless runner (E3-03). **Timings are config, never hardcoded (rule 6):** `TickProperties` (`stellar-compact.orchestrator.tick`) supplies `interval`, `negotiation-timeout`, `action-timeout` and `negotiation-rounds` — these are *orchestration* wall-clock values held entirely outside the deterministic engine boundary (principle 1), so changing a deadline can never change resolved state. The negotiation phase is structurally present here and gathers messages; routing them into the next `WorldView` and structured proposals are E4-06.
